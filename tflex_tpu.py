...
def while_loop(condition, body, inputs=None, arrays=None, infeed_queue=None, name=None):
  ...
  inputs = [] if inputs is None else [to_tensor(x) for
                                      x in inputs]
  arrays = [] if arrays is None else [to_tensor(x) for
                                      x in arrays]
  ...
  def body_wrapper(*inputs):
    ...
    output_arrays = [o for o in outputs if isinstance(o, tf.TensorArray)]
    outputs = [o for o in outputs if not isinstance(o, tf.TensorArray)]
    ...
    outputs = [
        o if isinstance(o, ops.Operation) else to_tensor(o)
        for o in outputs
    ]
    ...
    if tensor_tracer.TensorTracer.is_enabled():
      ...
      output_tensors = tt.trace_tpu(ops.get_default_graph(),
                                    output_tensors, None,
                                    num_replicas)
    return output_tensors + output_arrays
...
def repeat(n, body, inputs=None, arrays=None, infeed_queue=None, name=None):
  ...
  def body_wrapper(i, *args):
    return [i + 1] + _convert_to_list(body(i, *args))

  inputs = [0] if inputs is None else [0] + _convert_to_list(inputs)
  outputs = while_loop(
      cond, body_wrapper, inputs=inputs, arrays=arrays, infeed_queue=infeed_queue, name=name)
